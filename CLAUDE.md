...
**⚠️ Register is GENDERED (v1.3 fix — do not flatten):** acknowledgment particle = MALE `ขอรับ` / FEMALE `เจ้าค่ะ` (the period-correct female form — NOT the banned modern bare `ค่ะ`). Humble self-reference = MALE `ข้าน้อย` / FEMALE plain `ข้า` (**women never say `ข้าน้อย`**). `เจ้า`/`ท่าน`/`ข้า` are gender-neutral. Identify female speakers **from `tools/speaker_map.tsv`** (v1.5 / JOB 3) — it maps 55,192 dict lines to the character who actually says them, extracted from the game's own Lua story scripts; `tools/speaker_gender.tsv` holds the 343 speakers with their gender evidence. That beats every heuristic and supersedes them where they disagree. Fall back to the CN source pronoun in the key (`妾身`/`奴家`/`小女子`/`本宫` = female; `小人`/`小的` = male) only for lines with no speaker data — and note **`在下` is NOT male-only** (龙湘, female, uses it). A line with more than one speaker (generic `是。`/`好。`) is un-genderable: leave it male. So game-wide `ครับ`=0 and bare-`ค่ะ`=0, but `เจ้าค่ะ`>0 is CORRECT.
